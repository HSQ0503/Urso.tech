...
const managerCockpitSteps = [
  ...
  { featuredActionId: "EXE-02", placement: "completed", scenarioControl: null, milestoneStatus: "protected" },
];
...
export function deriveMfManagerCockpitPresentation(snapshot) {
  ...
  const featuredManagerAction = stepPresentation
    ...
    : null;
  const primaryManagerAction = stepPresentation?.placement === "actionable"
    && queue.now.length === 1
    && queue.now[0].actionId === stepPresentation.featuredActionId
    ? queue.now[0]
    : null;
  ...
  return {
    ...
    managerActionPlacement: stepPresentation?.placement ?? null,
    primaryManagerAction,
    scenarioControl: stepPresentation?.scenarioControl
