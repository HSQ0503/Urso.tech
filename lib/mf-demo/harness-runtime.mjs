...
function sourceAtStep(source, step) {
  let evidencePaths = [...source.evidencePaths];
  let freshness = source.freshness;

  if (source.id === "supplier-communication" && step < 1) {
    evidencePaths = [];
    freshness = { pt: "Monitoramento ativo", en: "Monitoring active" };
  } else if (source.id === "controlled-documents" && step < 1) {
    evidencePaths = evidencePaths.filter((path) => /revision b/i.test(path));
  } else if (source.id === "bim-models" && step < 4) {
    evidencePaths = evidencePaths.filter((path) => !/revision c/i.test(path));
  } else if (source.id === "project-schedule" && step < 6) {
    evidencePaths = evidencePaths.filter((path) => /baseline schedule/i.test(path));
  } else if (source.id === "rfi-decisions" && step < 2) {
    evidencePaths = [];
    freshness = { pt: "Nenhuma decisão aberta", en: "No decision open" };
  } else if (source.id === "rfi-decisions" && step >= 3) {
    freshness = { pt: "DEC-042 aprovada", en: "DEC-042 approved" };
  }

  return { ...source, evidencePaths, freshness };
}
...
function buildWorkItems(step) {
  ...
  return mfScenarioManifest.workflow.tasks.map((task) => {
    ...
    const activeAt = task.managerAction?.actionAt ?? Math.max(0, completeAt - 1);
    ...
    else if (dependenciesComplete && step >= activeAt) state = "in_progress";
    else if (dependenciesComplete && !task.managerAction) state = "ready";
    states.set(task.id, state);
    return {
      ...task,
      state,
      completeAt,
      receiptId: state === "complete"
        ? task.id === "release-exe-02" ? "RCPT-PILOT-CONTROL" : `RCPT-${task.id.toUpperCase()}`
        : null,
    };
  });
}
...
export function createMfHarnessSnapshot(step = 0) {
  ...
  return {
    ...
    truth: {
      ...
      revisionC: approved ? "current" : normalizedStep === 0 ? "not_received" : "unresolved",
    },
    decision: {
      id: mfScenarioManifest.decision.id,
      status: approved ? "approved" : normalizedStep === 0 ? "not_open" : "pending",
    ...
    sources: mfScenarioManifest.sources.map((source) => ({
      ...sourceAtStep(source, normalizedStep),
