...
function resolveSource(snapshot, sourceId, workflowId) {
  configuredReference(mfScenarioManifest.sources, sourceId, "source", workflowId);
  return snapshotReference(snapshot.sources, sourceId, "source", workflowId);
}
...
function resolveGateReceipt(snapshot, task) {
  if (task.state !== "complete") return { state: "pending", id: null };
  if (task.receiptId) return { state: "available", id: task.receiptId };

  const transitionReceipt = [...snapshot.receipts]
    .filter((receipt) => receipt.toStep >= task.completeAt)
    .sort((left, right) => right.toStep - left.toStep || left.id.localeCompare(right.id))[0];
  return transitionReceipt
    ? { state: "available", id: transitionReceipt.id }
    : { state: "missing", id: null };
}
...
function resolveRoleDelivery(snapshot, roleId, workflowId) {
  ...
  const sources = mfScenarioManifest.sources
    ...
    .map((source) => resolveSource(snapshot, source.id, workflowId));
...
export function deriveMfWorkflowPresentation(snapshot, workflowId) {
  ...
  const receipt = resolveGateReceipt(snapshot, gateTask);
  const gate = {
    task: gateTask,
