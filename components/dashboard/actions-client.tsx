            ...
            return (
              <div key={a.key} className={`flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 ${i > 0 ? "border-t border-edge" : ""}`}>
                ...
                <div className="flex items-center gap-2.5">
                  <Segmented
                    options={autonomyOptions.map((o) => ({ ...o, label: t(o.label) }))}
                    value={st.autonomy}
...
function ActionCard({ action: a, onApprove, onDismiss }: { action: ActionWithPlan; onApprove: () => void; onDismiss: () => void }) {
  ...
  const next = a.status !== "completed" ? t(actionStatusLabel[ACTION_FLOW[idx + 1]]) : null;
  ...
  return (
    <Card className="flex flex-col gap-4">
      ...
      <Modal
        ...
        eyebrow={`${a.agent} ${t("agent")} · ${a.store}`}
        title={a.title}
        footer={
          isSuggested ? (
            <div className="flex items-center gap-2">
