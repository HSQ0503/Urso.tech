...
// ChartInfo renders in BOTH server and client components and is synchronous, so
// neither getI18n nor useT is valid here. The translator is passed in as an
// optional prop; it falls back to identity (English). Guide copy lives in the
// plain-data chart-guides.ts, so it's translated at this render site via t(...).
...
export function ChartInfo({ id, align = "left", t = identity }: { id: GuideId; align?: keyof typeof ALIGN; t?: T }) {
  ...
  return (
    <span className="group/ci relative inline-flex align-middle">
      ...
      <span
        role="tooltip"
        className={`pointer-events-none absolute bottom-[calc(100%+8px)] z-50 w-[266px] rounded-lg border border-edge bg-panel-strong p-3.5 text-left opacity-0 shadow-[0_18px_44px_-16px_rgba(0,0,0,0.85)] transition-opacity duration-150 group-hover/ci:opacity-100 group-focus-within/ci:opacity-100 motion-reduce:transition-none ${ALIGN[align]}`}
      >
        <span className="block font-mono text-2xs uppercase tracking-[0.12em] text-ink-dimmer">{t("How to read this")}</span>
        <span className="mt-2 block font-sans text-xs font-medium normal-case leading-relaxed tracking-normal text-ink">{t(g.summary)}</span>
        <span className="mt-2 block font-sans text-xs normal-case leading-relaxed tracking-normal text-ink-dim">{t(g.read)}</span>
        {g.legend && (
          <span className="mt-3 block space-y-1.5 border-t border-edge pt-2.5">
            {g.legend.map((it) => (
              <span key={it.label} className="flex items-start gap-2">
                <Swatch item={it} />
                <span className="font-sans text-xs normal-case leading-[1.4] tracking-normal">
                  <span className="text-ink">{t(it.label)}</span>
                  {it.note && <span className="text-ink-dim"> — {t(it.note)}</span>}
                </span>
              </span>
            ))}
          </span>
        ...
        {g.source && (
          <span className="mt-2.5 block font-mono text-2xs uppercase tracking-[0.12em] text-ink-dimmer">{t(g.source)}</span>
        )}
      </span>
    </span>
  );
