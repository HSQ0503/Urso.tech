...
export function groomerColHelp(t: Translate = (s) => s) {
  return (
    <div>
      ...
      <ul className="mt-2 space-y-1.5">
        {COLS.map((c) => (
          <li key={c.label}>
            <span className="font-mono text-ink">{t(c.label)}</span>
            <span className="text-ink-dim"> — {t(c.desc)}</span>
          </li>
        ))}
      </ul>
    </div>
...
export const GROOMER_COL_HELP = groomerColHelp();
