...
function SubHead({ eyebrow, title, right }: { eyebrow: string; title: string; right?: React.ReactNode }) {
  return (
    <div className="mb-4 flex items-end justify-between gap-3">
      <div>
        <Micro>{eyebrow}</Micro>
        <h2 className="mt-1.5 text-base font-semibold tracking-[-0.01em]">{title}</h2>
      </div>
      {right}
    </div>
  );
          ...
          <Card>
            <div className="mb-4 flex items-start justify-between">
              <div>
                ...
                <div className="mt-1.5 text-2xl font-semibold tracking-[-0.01em] tabular-nums">{cs.total.toLocaleString()} <span className="text-sm text-ink-dim">{t("calls")}</span></div>
              </div>
              <div className="text-right">
                <div className="text-xl font-semibold tabular-nums text-bad">{pct(cs.missedPct)}</div>
          ...
          <Card className="flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-edge bg-edge">
              <div className="bg-panel p-3.5">
                ...
                <div className="mt-1.5 font-mono text-base text-ink">{(cs.total - cs.missed).toLocaleString()}</div>
              </div>
              <div className="bg-panel p-3.5">
                <Micro>{t("Missed")}</Micro>
                <div className="mt-1.5 font-mono text-base text-bad">{cs.missed.toLocaleString()}</div>
              </div>
            </div>
            <p className="mt-auto pt-4 text-xs leading-relaxed text-ink-dim">
          ...
          <Card>
            <div className="mb-4 flex items-end justify-between">
              <div>
                ...
                <div className="mt-1.5 text-sm text-ink-dim">{t("From first visit to a booked appointment")}</div>
              </div>
              <div className="text-right">
                <div className="text-xl font-semibold tabular-nums">{pct(ws.convRate, 1)}</div>
          ...
          <Card>
            <div className="mb-4 flex items-start justify-between">
              <div>
                ...
                <div className="mt-1.5 text-2xl font-semibold tracking-[-0.01em] tabular-nums">{ws.visits.toLocaleString()} <span className="text-sm text-ink-dim">{t("visits")}</span></div>
              </div>
              <div className="text-right">
                <div className="text-xl font-semibold tabular-nums">{ws.bookings.toLocaleString()}</div>
          ...
          <Card>
            <div className="mb-4 flex items-end justify-between">
              <div>
                ...
                <div className="mt-1.5 text-2xl font-semibold tracking-[-0.01em] tabular-nums">{fmtMoney(m.revenue)}</div>
              </div>
              <span className="font-mono text-2xs uppercase tracking-[0.12em] text-ink-dimmer">{period}</span>
            </div>
          ...
          <Card className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-4 border-t border-edge pt-5">
              <div>
                ...
                <div className="mt-1.5 font-mono text-base text-ink">{fmtMoney(m.grooming)}</div>
              </div>
              <div>
                <Micro>{t("Retail")}</Micro>
                <div className="mt-1.5 font-mono text-base text-ink">{fmtMoney(m.retail)}</div>
              </div>
              <div>
                <Micro>{t("Avg ticket")}</Micro>
                <div className="mt-1.5 font-mono text-base text-ink">{fmtMoney(m.avgTicket)}</div>
              </div>
              <div>
                <Micro>{t("Retail attach")}</Micro>
                <div className="mt-1.5 font-mono text-base" style={{ color: m.attach < 0.25 ? "var(--color-bad)" : undefined }}>{pct(m.attach)}</div>
              </div>
            </div>
          </Card>
        </div>
