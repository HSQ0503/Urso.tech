...
// its own design language (light, home-services), its own access gate.
...
const display = Sora({ subsets: ["latin"], weight: ["600", "700"], variable: "--font-canes-display" });
...
export default function CanesRootLayout({ children }: { children: React.ReactNode }) {
  return <div className={`canes ${display.variable} ${body.variable} min-h-screen`}>{children}</div>;
}
