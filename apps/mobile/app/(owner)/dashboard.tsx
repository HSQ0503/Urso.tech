...
//
// This WAS the home screen. Markate keeps its home as a launcher and its
// dashboard as a destination behind one tile, and since Sebastian's hands are
// trained on that split, ours moved to match (see app/(owner)/index.tsx). The
// content is unchanged: it is an ACTION QUEUE, not a report.
//
// Sebastian sells and dispatches; he does not wash. His day is catch the lead
// fast, get it on the calendar, keep people answered. So what needs him now is
// at the top with a Call button on it, and money is context underneath.
// Speed-to-lead is the metric that wins the job, so the call queue gets the only
// full-width orange control here.
//
// Every time is America/New_York via fmtEt. The device clock is never read —
// not for the date, not for a wait timer, not for grouping. A phone that has
// travelled must still show the same day as the office.
...
import { callActions } from "@/api";
import { keys, useAgenda, useOverview, useTodayReport } from "@/queries";
...
export default function DashboardScreen(): React.ReactElement {
  ...
  const bridgeCall = useAction(
    (input: { leadId: string; phone: string }) => callActions.bridge(input.phone, input.leadId),
    { invalidates: [keys.leads.all(), keys.threads.all(), keys.overview()] },
  );
  useRefetchOnFocus(overviewQuery.refetch);
  useRefetchOnFocus(agendaQuery.refetch);
  useRefetchOnFocus(reportQuery.refetch);
  const { refreshing, onRefresh } = usePullToRefresh(() =>
    Promise.all([overviewQuery.refetch(), agendaQuery.refetch(), reportQuery.refetch()]),
  );
  ...
  const reportNotice = noticeFrom(reportQuery.error);
  if (reportNotice !== null && !notices.includes(reportNotice)) notices.push(reportNotice);
  ...
  // and computing minutesSince() only at render meant a queue left open kept
  // showing a lead's original minute count in its original colour: one that had
  // crossed fifteen minutes still read calm. Ticking locally costs nothing and
  // is the difference between a live queue and a screenshot of one.
  useFocusEffect(
    useCallback(() => {
  ...
  const callLead = useCallback((leadId: string, phone: string) => {
    ...
    void bridgeCall.mutateAsync({ leadId, phone }).then((result) => {
      if (!result.ok) setCallFailedFor(leadId);
    });
  }, [bridgeCall]);

  // Hold the full-screen spinner until BOTH cold-start loads settle — dropping
  // it on the first answer painted a screen missing its "Needs you now" half
  // for the slower read's whole round trip. errorUpdateCount keeps this a
  // COLD-START gate only: a no-data refetch resets status to pending (v5
  // fetchState), so without it a permission-refused overview would throw this
  // spinner over a perfectly usable agenda on every focus.
  const coldStart = (q: { isPending: boolean; errorUpdateCount: number }) =>
    q.isPending && q.errorUpdateCount === 0;
  const showSpinner = coldStart(overviewQuery) || coldStart(agendaQuery) || coldStart(reportQuery);
  ...
  return (
    <View style={styles.screen}>
      <ChromeBar
        title="Dashboard"
        sub={fmtEt(new Date().toISOString(), {
          weekday: "short",
          month: "long",
          day: "numeric",
        })}
      ...
      {showSpinner ? (
        ...
        >
          {/* Edge to edge and flush against the chrome. A zero renders faint —
              an amount that has not happened yet should not read as a figure. */}
          {overview !== null ? (
            <MoneyStrip
          ...
          <View style={styles.body}>
            ...
            {/* Today's report — the six ET-calendar-day figures, in Markate's
                two-column grid. Every one comes from /canes/today-report, whose
                window is the DAY; nothing here is derived from overview, whose
                money is rolling 7-day. A "today" card must never be fed a
                week's number just because the field was nearby. */}
            {report !== null ? (
              <Section label="Today's report" meta={report.dayLabel}>
                <StatGrid
                  cards={[
                    {
                      key: "jobsToday",
                      label: "Today's jobs",
                      icon: "briefcase",
                      value: fmtMoney(report.jobsToday.cents),
                      count: report.jobsToday.count,
                      dim: report.jobsToday.count === 0,
                    },
                    {
                      key: "earned",
                      label: "Earned today",
                      icon: "dollar-sign",
                      value: fmtMoney(report.earnedTodayCents),
                      dim: report.earnedTodayCents === 0,
                    },
                    {
                      key: "booked",
                      label: "Jobs booked today",
                      icon: "check-square",
                      value: fmtMoney(report.bookedToday.cents),
                      count: report.bookedToday.count,
                      dim: report.bookedToday.count === 0,
                    },
                    {
                      key: "tomorrow",
                      label: "Tomorrow's jobs",
                      icon: "sunrise",
                      value: fmtMoney(report.tomorrowJobs.cents),
                      count: report.tomorrowJobs.count,
                      dim: report.tomorrowJobs.count === 0,
                    },
                    {
                      key: "estimates",
                      label: "Today's estimates",
                      icon: "file-text",
                      value: fmtMoney(report.estimatesToday.cents),
                      count: report.estimatesToday.count,
                      dim: report.estimatesToday.count === 0,
                    },
                    {
                      key: "leads",
                      label: "Today's leads",
                      icon: "filter",
                      // A bare count IS the figure here, so no "(n)" qualifier.
                      value: String(report.leadsTodayCount),
                      dim: report.leadsTodayCount === 0,
                    },
                  ]}
                />
              </Section>
...
const styles = StyleSheet.create({
  ...
  trailing: { ...type.ruleSm, color: color.muted },

  wait: { fontFamily: font.mono, fontSize: 13, fontVariant: ["tabular-nums"] },
  waitOk: { color: color.muted },
  waitWarn: { color: color.brandDeep },
